package com.personal.business.employeescale.update.services;

import java.util.Optional;

import com.personal.business.employeescale.entities.EmployeeScale;
import com.personal.business.employeescale.factories.EmployeeScaleResultFactory;
import com.personal.shared.query.Query;
import com.personal.shared.services.IEditService;
import com.personal.shared.services.ServiceResult;

import io.helidon.dbclient.DbClient;

public class EditEmployeeScaleService implements IEditService<EmployeeScale> {

    private final Optional<DbClient> dbClient;

    public EditEmployeeScaleService(Optional<DbClient> dbClient) {
        this.dbClient = dbClient;
    }

    @Override
    public ServiceResult<EmployeeScale> edit(Query query) {

        if (dbClient.isEmpty()) {
            return EmployeeScaleResultFactory.UpdateFail();
        }

        var dbclient = dbClient.get();

        var updateQuery = query.Update("employee_scale").Get();
        //System.out.println("updateQuery: " + updateQuery);

        long result = 0;
        try {
            result = dbclient.execute()
                    .createUpdate(updateQuery)
                    .params(query.getParams())
                    .execute();

        } catch (Exception e) {
            //System.out.println("Hubo una excepción al actualizar el tipo de usuario " + e.getMessage());
            return EmployeeScaleResultFactory.UpdateFail();
        }

        //System.out.println("query.getParams(): " + query.getParams().values().toString());
        if (result == 0) {
            return EmployeeScaleResultFactory.UpdateFail();
        }

        return EmployeeScaleResultFactory.UpdateSuccess(new EmployeeScale());

    }

}
