    ...
    public ServiceResult<List<OrgRelation>> filter(Query query) {
        ...
        var result = this.dbClient.get().execute()
                ...
                .map((dbRow) -> {
                    ...
                    or.setStructure(new SharedOrgStructure(dbRow.column("organization_structure").getString()));
                    or.setHierarchy(new SharedOrgHierarchy(dbRow.column("organization_hierarchy").getString()));
